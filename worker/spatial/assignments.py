...
def _majority_overlap_sequential(
    ...
    if default_value is not None:
        assignments[overlay_attr_col] = assignments[overlay_attr_col].fillna(default_value)
...
def _process_overlap_chunk(
    ...
    return _majority_overlap_sequential(
        input_chunk, overlay_gdf, input_id_col, overlay_attr_col, output_field, default_value
    )
...
def any_intersection(
    ...
    intersections = gpd.sjoin(input_gdf, overlay_gdf, how="left", predicate="intersects")
    ...
    result[output_field] = result[output_field].apply(lambda x: x if isinstance(x, list) else [])
    ...
    **kwargs,
    ...
    if strategy == "majority_overlap":
        return majority_overlap(
            input_gdf, overlay_gdf, input_id_col, overlay_attr_col, output_field, **kwargs
        )
