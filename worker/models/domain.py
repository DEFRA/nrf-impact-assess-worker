...
class WastewaterImpact(BaseModel):
    ...
    occupancy_rate: float | None = Field(
        default=None, gt=0, description="People per dwelling (None if rates unavailable)"
    )
    water_usage_L_per_person_day: float | None = Field(
        default=None, gt=0, description="Litres per person per day (None if rates unavailable)"
    )
    daily_water_usage_L: float | None = Field(
        default=None, ge=0, description="Total daily water usage (None if rates unavailable)"
    )
    nitrogen_conc_2025_2030_mg_L: float | None = Field(
        default=None, description="N concentration 2025-2030 (mg/L)"
    )
    phosphorus_conc_2025_2030_mg_L: float | None = Field(
        default=None, description="P concentration 2025-2030 (mg/L)"
    )
    nitrogen_conc_2030_onwards_mg_L: float | None = Field(
        default=None, description="N concentration 2030+ (mg/L)"
    )
    phosphorus_conc_2030_onwards_mg_L: float | None = Field(
...
class GcnDevelopment(BaseModel):
    ...
    orig_fid: int = Field(ge=0, description="Original feature ID from input RLB GeoDataFrame")
...
class GcnAssessmentResult(BaseModel):
    ...
    # Primary outputs
    habitat_impacts: list[GcnHabitatImpact] = Field(description="Habitat impacts by risk zone")
